...
export class WorkPage {
  ...
  readonly tags = Array.from(new Set(PROJECTS.map((project) => project.domain))).sort();
  activeTag = 'All';
  selectedProject?: Project;

  get filteredProjects(): Project[] {
    return this.activeTag === 'All' ? this.projects : this.projects.filter((project) => project.domain === this.activeTag);
  }

  constructor() {
    this.seo.update({
      title: 'Work | Lokesh Paduchuri',
      description: 'Modal-driven case studies for frontend architecture, platform strategy, and delivery outcomes.'
    });
    this.seo.setJsonLd({
      '@context': 'https://schema.org',
      '@type': 'ItemList',
      itemListElement: PROJECTS.map((project, index) => ({ '@type': 'CreativeWork', position: index + 1, name: project.title }))
    });
